...
## What to do
1. **`src/state/AppContext.tsx`** — inside `AppProvider`, add `useState` for `model` and `messages`, implement `sendMessage(text)` (call `sendMessageApi(text, model)` from `src/api.ts` — same append-then-simulate-a-reply approach as Lesson 1, but this `sendMessageApi` also takes the active `model`, and append the reply in its `.then()`), and provide it all via `<AppContext.Provider value={{ ... }}>`.
